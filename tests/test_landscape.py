"""Research landscape parity tests."""
...
def test_research_landscape_records_empty_query_coverage_gap() -> None:
    payload = build_research_landscape(
        [
            ScanBatch(_search("empty query", []), path="empty.json"),
            ScanBatch(_search("populated query", [_row("P1", "n1", 0.4, title="Paper One")])),
        ],
        pull_budget=3,
    )

    assert payload["pull_budget"] == 3
    assert payload["candidate_coverage_gaps"] == [
        {
    ...
    assert payload["coverage_map"]["under_covered_regions"] == payload["candidate_coverage_gaps"]
